...
public class CastTO implements Comparable<CastTO> {
    private int id = -1;
    ...
    public CastTO(String castJsonTxt) {
        super();
        try {
            castNode = (ObjectNode)jsonMapper.readTree(castJsonTxt);
    ...
    public ObjectNode geCastJson() {
        this.castNode = new ObjectNode(factory);
        ObjectNode castMovieNode = null;
        ...
        if (StringUtil.isNotEmpty(this.character) && this.order > 0) {            
            ...
            for (CastMovieTO castMovieTO : this.getCastMovieList()) {
                castMovieNode = new ObjectNode(factory);
